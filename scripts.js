...
// Evitar agregar múltiples eventos 'change'
dropDownNombrePPFF.off('change'); // Elimina cualquier evento 'change' anterior
...
dropDownNombrePPFF.on('change', function () {
    ...
    let content = `<h6><i>Inmuebles <b>(${features.length})</b></h6></i>`;
    ...
    features.forEach((feature, index) => {
        ...
        const superficieM2 = feature.properties.SUP_M2;
        const superficieHa = feature.properties.SUP_HA;
        const descripcion = feature.properties.DESCRIPCIO;
        const clasificacion = feature.properties.CLASIFICA;
        const tenencia = feature.properties.TENENCIA;
        ...
        if (nombre || region || comuna) {
            ...
            let cardContent = `
                <div class="card ficha${isFirstElement ? ' first-element' : ''}" onclick="centrarMapa('${featureId}')">
                    ...
                    ${superficieHa ? `<p><span class="etiqueta">Superficie Hectáreas</span> ${superficieHa}</p>` : ''}
                    ${clasificacion ? `<p><span class="etiqueta">Clasificación</span> ${clasificacion}</p>` : ''}
                    ${tenencia ? `<p><span class="etiqueta">Tipo de tenencia</span> ${tenencia}</p>` : ''}
